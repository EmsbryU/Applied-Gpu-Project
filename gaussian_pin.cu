...
int main(int argc, char *argv[])
{
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
	...
	int *dummy;
	cudaHostAlloc((void**)&dummy, sizeof(float), cudaHostAllocDefault);
    ...
    for(i=1;i<argc;i++) {
      if (argv[i][0]=='-') {// flag
        flag = argv[i][1];
		  ...
		  cudaHostAlloc((void**)&a, Size * Size * sizeof(float), cudaHostAllocDefault);
	      //a = (float *) malloc(Size * Size * sizeof(float));
	      create_matrix(a, Size);

		  cudaHostAlloc((void**)&b, Size * sizeof(float), cudaHostAllocDefault);
