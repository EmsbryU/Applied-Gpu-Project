...
if [ "$MATRIX_SIZE" != "4" ] && [ "$MATRIX_SIZE" != "16" ] && [ "$MATRIX_SIZE" != "1024" ] && [ "$MATRIX_SIZE" != "4096" ]; then
...
OLD_FILTERED_FILE="$RESULTS_DIR/filteredOldMat${MATRIX_SIZE}.txt"
IMP_FILTERED_FILE="$RESULTS_DIR/filteredImpMat${MATRIX_SIZE}.txt"
...
# Extract relevant lines starting after "The final solution is: "
echo "Filtering results for comparison..."
awk '/The final solution is:/{flag=1; next} flag' $OLD_RESULT_FILE > $OLD_FILTERED_FILE
awk '/The final solution is:/{flag=1; next} flag' $IMP_RESULT_FILE > $IMP_FILTERED_FILE

# Compare results
echo "Comparing filtered results for matrix size $MATRIX_SIZE..."
diff $OLD_FILTERED_FILE $IMP_FILTERED_FILE || echo "Differences found between results."
