...
#else
        #define MAXBLOCKSIZE 512 					//定义一维计算的线程块大小 BlockSize
#endif
...
#else
        #define BLOCK_SIZE_XY 4						//定义二维计算的线程块大小 BlockSize
#endif
...
// void invertMatrix(float *d_A, float *d_Ainv, int n);
// void matrixMultiply(float *d_A, float *d_B, float *d_C, int n);
void BackSubWithCuSolver(float *a, float *b, float *finalVec, int Size);

unsigned int totalKernelTime = 0;

// create both matrix and right hand side, Ke Wang 2013/08/12 11:51:06

// 初始化的m矩阵，不管size多少，对角线上都是10，其余两边是对称的，是指数衰减函数的系数
void
...
// 读取-s 后的 size参数，然后用creat_matrix创建系数矩阵 a, 再创建 结果向量b（均为0）和 分配 a,b,m的内存

// 读取-q verbose =0 将不打印 矩阵和结果
    ...
    InitPerRun();     					//初始化m 矩阵，所有元素设置为 0.0
    //begin timing
    ...
    BackSubWithCuSolver(a, b, finalVec, Size);
    if (verbose) {
...
//Fan1 更新 m_cuda 的第 (idx+t+1)行，第 t列
			//对应的是a_cuda 矩阵的 第(idx+t+1)行 t列 / 第 t 行 t列
...
void ForwardSub()
{
	...
	grid_size = (Size/block_size) + (!(Size%block_size)? 0:1); //向上取整
...
void BackSubWithCuSolver(float *a, float *b, float *finalVec, int Size) {
    ...
    // Query working space of getrf and getrs
    cusolverDnSgetrf_bufferSize(cusolverH, Size, Size, d_A_T, Size, &lwork);
    cudaMalloc((void**)&d_work, lwork * sizeof(float));

    // LU factorization
    cusolverDnSgetrf(cusolverH, Size, Size, d_A_T, Size, d_work, d_pivot, d_info);

    // Solve the linear system
    cusolverDnSgetrs(cusolverH, CUBLAS_OP_N, Size, 1, d_A_T, Size, d_pivot, d_b, Size, d_info);
