...
__global__ void Fan3(float *final, float *a, float *b,int Size);
...
void SolveUpperTriangular(float *a, float *b, float *finalVec, int Size);

struct timespec time_whole_start;
struct timespec time_whole_end;

struct timespec time_start_forward;
struct timespec time_end_forward;
...
int main(int argc, char *argv[])
{
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
	...
	cudaHostAlloc((void**)&dummy, Size * Size * sizeof(float), cudaHostAllocDefault);   
    clock_gettime(CLOCK_MONOTONIC, &time_whole_start);
    ...
    for(i=1;i<argc;i++) {
      ...
      if (argv[i][0]=='-') {// flag
        flag = argv[i][1];
		  ...
		  //cudaHostAlloc((void**)&b, Size * Size * sizeof(float), cudaHostAllocDefault);
	      b = (float *) malloc(Size * sizeof(float));
	      for (j =0; j< Size; j++)
	    	b[j]=1.0;

	      //cudaHostAlloc((void**)&m, Size * Size * sizeof(float), cudaHostAllocDefault);
    ...
    InitPerRun();
    //begin timing
    clock_gettime(CLOCK_MONOTONIC, &time_start_forward);

    // run kernels
    ForwardSub();
    
    //end timing
    clock_gettime(CLOCK_MONOTONIC, &time_end_forward);
...
/*     if (verbose) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_whole_end);

    //forward function
	printf("Time total (including memory transfers):\t%f sec\n", double(time_end_forward.tv_sec - time_start_forward.tv_sec) + (time_end_forward.tv_nsec - time_start_forward.tv_nsec) / 1000000000.0);

	//backward kernel execution time
	printf("Time kernel of backsub:\t%f sec\n", double(time_end_backsub.tv_sec - time_start_backsub.tv_sec) + (time_end_backsub.tv_nsec - time_start_backsub.tv_nsec) / 1000000000.0);

	//forward function execution time
	...
	//whole including for loop in main
	printf("Time for whole main function:\t%f sec\n", double(time_whole_end.tv_sec - time_whole_start.tv_sec) + (time_whole_end.tv_nsec - time_whole_start.tv_nsec) / 1000000000.0);
...
__global__ void Fan3(float *final_cuda, float *a_cuda, float *b_cuda,int Size)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if(idx >= Size) return;

	final_cuda[Size-idx-1]=b_cuda[Size-idx-1];
	for(int j=0;j<idx;j++)
	{
		final_cuda[Size-idx-1]-=*(a_cuda+Size*(Size-idx-1)+(Size-j-1)) * final_cuda[Size-j-1];
	}
	final_cuda[Size-idx-1]=final_cuda[Size-idx-1]/ *(a_cuda+Size*(Size-idx-1)+(Size-idx-1));
}
...
void SolveUpperTriangular(float *a, float *b, float *finalVec, int Size) {
    ...
    cudaMalloc((void**)&d_A_T, Size * Size * sizeof(float));
    ...
    cublasSgeam(cublasH, CUBLAS_OP_T, CUBLAS_OP_N, Size, Size, &alpha, d_A, Size, &beta, d_A, Size, d_A_T, Size);


    // Solve the linear system using cublasStrsm
	clock_gettime(CLOCK_MONOTONIC, &time_start_backsub);
    cublasStrsm(cublasH, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, Size, 1, &alpha, d_A_T, Size, d_b, Size);
	
	cudaDeviceSynchronize();
	clock_gettime(CLOCK_MONOTONIC, &time_end_backsub);

    // Copy result back to host
    cudaMemcpy(finalVec, d_b, Size * sizeof(float), cudaMemcpyDeviceToHost);
