...
void BackSub()
{
	float *a_cuda,*b_cuda,*final_cuda;

	cudaMalloc((void **) &a_cuda, Size * Size * sizeof(float));
	 
	cudaMalloc((void **) &b_cuda, Size * sizeof(float));
	
	cudaMalloc((void **) &final_cuda, Size * sizeof(float));	
	...
	Fan3<<<dimGrid,dimBlock>>>(final_cuda,a_cuda,b_cuda,Size);
	cudaDeviceSynchronize();
	clock_gettime(CLOCK_MONOTONIC, &time_end_backsub);

	cudaMemcpy(finalVec, final_cuda, Size * sizeof(float), cudaMemcpyDeviceToHost);
