...
__global__ void Fan2NewNew(float *m_cuda, float *a_cuda, int Size, int t)
{
...
void ForwardSub()
{
	...
	cudaMalloc((void **)&m_cuda, Size * sizeof(float));
	...
	// copy memory to GPU
	cudaMemcpy(m_cuda, m, Size * sizeof(float), cudaMemcpyHostToDevice);
